...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const deviceConfig = {
        ...
        p6: { name: "P6: 超滤膜反洗", devices: { "FIT601": "流量计，UF反洗流量计。", "P601": "水泵，将水从RO渗透水箱泵至原水箱。", "P602": "水泵，从UF反洗水箱泵水以清洁膜。", "P603": "尚未在SWaT中实施。" }}
    };

    function getDeviceType(id) { return id.match(/^[A-Z]*/)[0]; }
    function getDeviceIcon(type) {
        const icons = { P: 'fa-solid fa-circle-chevron-right', MV: 'fa-solid fa-git-compare', UV: 'fa-solid fa-sun', LIT: 'fa-solid fa-ruler-vertical', FIT: 'fa-solid fa-gauge-high', AIT: 'fa-solid fa-flask-vial', DPIT: 'fa-solid fa-arrows-left-right-to-line', PIT: 'fa-solid fa-tachograph-digital', UNKNOWN: 'fa-solid fa-question-circle' };
    ...
    function buildDashboardAndFilters() {
        ...
        for (const stageId in deviceConfig) {
            ...
            dashboardHtml += `<div class="stage-card" id="card-${stageId}"><div class="stage-header"><h2>${stage.name}</h2></div><div class="device-group">`;
    ...
    async function updateDisplay(index, forceNoDiff = false) {
        ...
        if (isNaN(index) || index < 0 || index >= totalRecords) return;
        try {
            ...
            timestampDisplay.textContent = timestampData.Timestamp.replace(' AM', '').replace(' PM', '');
            ...
            if (attackInfo.isActive) {
                ...
                attackInfoWrapper.style.display = 'inline';
                ...
                attackInfoWrapper.style.display = 'none';
            }
    ...
    function addFilterEventListeners() {
        ...
        document.querySelectorAll('.stage-filter-checkbox').forEach(stageCb => {
            stageCb.addEventListener('change', (e) => {
                ...
                document.querySelectorAll(`.filter-checkbox[data-stage="${stageId}"]`).forEach(deviceCb => {
                    deviceCb.checked = e.target.checked;
                });
            });
        });
